...
#ifndef lint
static char rcsid[] MTR_UNUSED = "$Id: mtrBasic.c,v 1.1 2002/09/17 21:59:06 dlm Exp $";
#endif
  ...
  MtrNode * node)
{
    if (node == NULL) return;
    (void) fprintf(stdout,
        "N=%-8p C=%-8p Y=%-8p E=%-8p P=%-8p F=%x L=%d S=%d\n",
        node, node->child, node->younger, node->elder,
        node->parent, node->flags, node->low, node->size);
