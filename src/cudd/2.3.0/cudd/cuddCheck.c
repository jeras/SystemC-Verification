  ...
  DdManager * table)
{
	...
	while (f != NULL) {
		...
		if (f->index != CUDD_MAXINDEX) {
		    ...
		    fprintf(table->err,
			    "       node %p, id = %d, ref = %d, value = %g\n",
			    f,f->index,f->ref,cuddV(f));
		    flag = 1;
		}
	    } else {
		deadNode++;
	...
	if (count > (int)(f->ref) && f->ref != DD_MAXREF) {
	    fprintf(table->err,"ref count error at node %p, count = %d, id = %d, ref = %d, then = %p, else = %p\n",f,count,f->index,f->ref,cuddT(f),cuddE(f));
  ...
  DdManager * dd)
{
    ...
    /* Print header. */
    retval = fprintf(dd->out,"*** DD heap profile for %p ***\n", dd);
...
{
    f = Cudd_Regular(f);
    (void) fprintf(fp,"       node %p, id = %d, ref = %d, then = %p, else = %p\n",f,f->index,f->ref,cuddT(f),cuddE(f));
  ...
  DdNode * node)
{
	...
	for (j=0;j<slots;j++) {
		...
		if (cuddT(f) == node || Cudd_Regular(cuddE(f)) == node) {
		    (void) fprintf(table->out,"parent is at %p, id = %d, ref = %d, then = %p, else = %p\n",
			f,f->index,f->ref,cuddT(f),cuddE(f));
		}
		f = f->next;
	    }
